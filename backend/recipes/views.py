...
class TagView(viewsets.ModelViewSet):
    queryset = models.Tag.object.all()
...
class FavoriteView(APIView):
    permissins = [IsAuthenticatedOrReadOnly, ]
    ...
    def post(self, request, recipe_id):
        ...
        if models.Favorite.objects.filter(
            ...
            return Response(
                {'Ошибка': 'Уже находится в избранном'},
...
def download_shopping_cart(request):
    ...
    for record in shopping_cart:
        ...
        ingredients = models.IngredientAmount.objects.filter(recipe=recipe)
    ...
    response = HttpResponse(wishlist, content_type='text/plaint')
    return response
